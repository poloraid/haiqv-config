...
export KUBEFLOW_DEEPOPS_DEX_CONFIG="${KUBEFLOW_DEEPOPS_DEX_CONFIG:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/dex-config-map-haitf.yaml}"
export KUBEFLOW_DEEPOPS_DEX_DEPLOYMENT_PATCH="${KUBEFLOW_DEEPOPS_DEX_DEPLOYMENT_PATCH:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/deployment-dex-patch.yaml}"
export KUBEFLOW_DEEPOPS_DEX_KUSTOMIZE="${KUBEFLOW_DEEPOPS_DEX_KUSTOMIZE:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/kustomize-dex.yaml}"
export KUBEFLOW_DEEPOPS_USERNS_PARAMS="${KUBEFLOW_DEEPOPS_USERNS_PARAMS:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/user-namespace-params-haitf.env}"
export KUBEFLOW_DEEPOPS_CUSTOM_EXAMPLE="${KUBEFLOW_DEEPOPS_CUSTOM_EXAMPLE:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/kustomize-haitf.yaml}"
export KUBEFLOW_DEEPOPS_LOGOS_CONFIGMAP="${KUBEFLOW_DEEPOPS_LOGOS_CONFIGMAP:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/logos-configmap.yaml}"
export KUBEFLOW_DEEPOPS_CENTRALDASHBOARD_VS="${KUBEFLOW_DEEPOPS_CENTRALDASHBOARD_VS:-${KUBEFLOW_DEEPOPS_CONFIG_DIR}/centraldashboard-vs.yaml}"

# [haitf] haiqv portal kustomize
export HAIQV_PORTAL_MANIFEST="${HAIQV_PORTAL_MANIFEST:-${CONFIG_DIR}/files/haiqv}"
...
function help_me() {
  ...
  echo "-a    haitf-옵션, manifest를 새로 다운받아 설치 진행"
}
...
# [haitf] 기존 repo 삭제 function 생성
function delete_exist_repo() {
  pushd .
  if [ -d "${KUBEFLOW_MANIFESTS_DEST}" ]; then
    echo "Kubeflow manifests directory already exists at: ${KUBEFLOW_MANIFESTS_DEST}"
    echo "Delete this Repo & re-deploying"
    rm -rf ${KUBEFLOW_MANIFESTS_DEST}
    exit 1
  fi
}
...
function clone_repo() {
  ...
  cp -v "${KUBEFLOW_DEEPOPS_DEX_KUSTOMIZE}" "${KUBEFLOW_MANIFESTS_DEST}/common/dex/overlays/istio/kustomization.yaml"
  cp -v "${KUBEFLOW_DEEPOPS_DEX_DEPLOYMENT_PATCH}" "${KUBEFLOW_MANIFESTS_DEST}/common/dex/overlays/istio/deployment-dex-patch.yaml"

  # [haitf] kubeflow custom manifest
  cp -v "${KUBEFLOW_DEEPOPS_CUSTOM_EXAMPLE}" "${KUBEFLOW_MANIFESTS_DEST}/example/kustomization.yaml"

  # [haitf] jupyter logos configmap
  cp -v "${KUBEFLOW_DEEPOPS_LOGOS_CONFIGMAP}" "${KUBEFLOW_MANIFESTS_DEST}/apps/jupyter/jupyter-web-app/upstream/base/configs/logos-configmap.yaml"

  # [haitf] centraldashboard virtualservice
  cp -v "${KUBEFLOW_DEEPOPS_CENTRALDASHBOARD_VS}" "${KUBEFLOW_MANIFESTS_DEST}/apps/centraldashboard/upstream/overlays/istio/virtual-service.yaml"

  # [haitf] haiqv-portal kustomization
  cp -v -r "${HAIQV_PORTAL_MANIFEST}" "${KUBEFLOW_MANIFESTS_DEST}/apps/haiqv"
...
if [ ${KUBEFLOW_PRINT} ] && [ ${KUBEFLOW_DELETE} ]; then
  ...
  clone_repo
elif [ ${KUBEFLOW_DELETE_REDEPLOY} ]; then
