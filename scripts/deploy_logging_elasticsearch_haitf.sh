...
app_name="${ELASTIC_SEARCH_APP_NAME:-elasticsearch}"
app_version="${ELASTIC_VERSION:-7.10.2}"

# Allow overriding config dir to look in
export KUBEFLOW_DEEPOPS_ELASTIC_SEARCH_CONFIG="${KUBEFLOW_DEEPOPS_ELASTIC_SEARCH_CONFIG:-${CONFIG_DIR}/files/logging/elasticsearch}"
export ELASTIC_SEARCH_CUSTOM_VALUES="${ELASTIC_SEARCH_CUSTOM_VALUES:-${KUBEFLOW_DEEPOPS_ELASTIC_SEARCH_CONFIG}/values-haitf.yaml}"
...
if ! helm status "${app_name}" >/dev/null 2>&1; then
	helm repo update
	helm upgrade --install --wait "${app_name}" --version ${app_version} ${repo_name}/${app_name} -f ${ELASTIC_SEARCH_CUSTOM_VALUES} --create-namespace --namespace logging
fi
