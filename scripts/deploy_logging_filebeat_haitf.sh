...
app_name="${FILEBEAT_APP_NAME:-filebeat}"
app_version="${ELASTIC_VERSION:-7.10.2}"

# Allow overriding config dir to look in
export KUBEFLOW_DEEPOPS_FILEBEAT_CONFIG="${KUBEFLOW_DEEPOPS_FILEBEAT_CONFIG:-${CONFIG_DIR}/files/logging/filebeat}"
export FILEBEAT_CUSTOM_VALUES="${FILEBEAT_CUSTOM_VALUES:-${KUBEFLOW_DEEPOPS_FILEBEAT_CONFIG}/values-haitf.yaml}"
...
if ! helm status "${app_name}" >/dev/null 2>&1; then
	helm repo update
	helm upgrade --install --wait "${app_name}" --version ${app_version} ${repo_name}/${app_name} -f ${FILEBEAT_CUSTOM_VALUES} --create-namespace --namespace logging
fi
